...
def train_model(ckpt_name_cnn):
    ...
    model_cnn.add_module(
        "conv4", nn.Conv2d(in_channels=64, out_channels=128, kernel_size=3)
    )
    ...
    model_cnn.add_module(
        "fc_logits", nn.Linear(EMBEDDING_SIZE, NUM_CLASSES, bias=False)
    )

    device = torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")
    opt = torch.optim.Adam(model_cnn.parameters(), lr=LR)
    opt.zero_grad()

    if not os.path.exists("./mlops/bin"):
